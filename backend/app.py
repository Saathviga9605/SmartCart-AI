...
app = Flask(__name__)
# Allow all origins for development to fix "Failed to fetch" on Web
...
# Legacy Compatibility Aliases for SmartPantryApi
@app.route('/api/get-recipes', methods=['POST', 'OPTIONS'])
def legacy_get_recipes():
    from routes.recipes import recommend_recipes
    return recommend_recipes()

@app.route('/api/recommend-more', methods=['POST', 'OPTIONS'])
def legacy_recommend_more():
    from routes.recipes import recommend_recipes
    return recommend_recipes()
