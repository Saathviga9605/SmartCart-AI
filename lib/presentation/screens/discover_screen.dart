...
class _DiscoverScreenState extends State<DiscoverScreen> {
  ...
  final List<_CategoryData> _categories = [
    ...
    _CategoryData(
        'Beverages', Icons.local_cafe, AppColors.categoryBeverages, 52),
  ...
  void _loadAndListen() {
    ...
    grocery.addListener(_handleGroceryChange);
  }

  void _handleGroceryChange() {
    if (mounted) {
      _refreshSmartPantry();
    }
  }
  ...
  @override
  void dispose() {
    context.read<GroceryProvider>().removeListener(_handleGroceryChange);
    super.dispose();
  }

  Future<void> _refreshSmartPantry() async {
    final grocery = context.read<GroceryProvider>();
    final pantry = context.read<SmartPantryProvider>();
    final ingredients = grocery.activeItems.map((e) => e.name).toList();
    await pantry.refreshFromIngredients(ingredients);
  }
    ...
    final actions = [
      _QuickAction(
        ...
        onTap: () {
          ...
          if (_recipeKey.currentContext != null) {
            Scrollable.ensureVisible(
              _recipeKey.currentContext!,
              alignment: 0.0,
        ...
        children: actions.map((action) {
          return InkWell(
            ...
            child: Column(
              ...
              .animate()
              .fadeIn(
                  delay: Duration(milliseconds: actions.indexOf(action) * 100))
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemCount:
                pantry.suggestions.isNotEmpty ? pantry.suggestions.length : 5,
            itemBuilder: (context, index) {
              if (pantry.suggestions.isEmpty) {
                return Container(
                  width: 160,
              ...
              return SmartSuggestionCard(
                suggestion: pantry.suggestions[index],
              )
                  .animate()
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ListView.builder(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            scrollDirection: Axis.horizontal,
            itemCount: pantry.recipes.isNotEmpty ? pantry.recipes.length : 4,
            itemBuilder: (context, index) {
              if (pantry.recipes.isEmpty) {
                return Container(
                  width: 200,
                  margin: const EdgeInsets.only(right: 12),
                  child: const ShimmerLoading(
              ...
              final recipe = pantry.recipes[index];
              return RecipeSuggestionCard(
                recipe: recipe,
                onLike: () async {
                  await context.read<SmartPantryProvider>().sendRecipeFeedback(
  ...
  void _showBudgetDialog(BuildContext context) {
    final grocery = context.read<GroceryProvider>();
    final stats = grocery.itemsByCategory;
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            const Icon(Icons.analytics, color: AppColors.success),
            const SizedBox(width: 12),
            Text('Spending Insights', style: AppTextStyles.titleMedium),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Current List Breakdown', style: AppTextStyles.bodySmall.copyWith(color: AppColors.textSecondary)),
            const SizedBox(height: 16),
            if (stats.isEmpty)
              Text('Add items to your list to see insights!', style: AppTextStyles.bodySmall)
            else
              ...stats.entries.map((e) => Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(child: Text(e.key.displayName, style: AppTextStyles.bodySmall)),
                    Text('${e.value.length} items', style: AppTextStyles.labelSmall),
                  ],
                ),
              )).toList(),
          ],
  ...
  void _shareCurrentList(BuildContext context) {
    final grocery = context.read<GroceryProvider>();
    final active = grocery.activeItems;
    ...
    final String listText = active.map((e) => '• ${e.quantity}x ${e.name}').join('\n');
    final String shareText = 'My SmartCart Shopping List:\n\n$listText\n\nShared via SmartCart AI';
    
    // In a real app we'd use Share.share(shareText);
    // For demo, we'll show a "Share Sheet" dialog
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Text(shareText, style: AppTextStyles.bodySmall),
            ),
